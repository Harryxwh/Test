...
public:
	...
	static int eval_site_index(int sl_index, const std::vector<int>& n) {
		 ...
		 */
		assert(n[dim - 1] < L[dim - 1] && n[dim - 1] >= 0);
		int s_idx = n[dim - 1];
...
private:
	...
	std::vector<int> NN_site_index; /* nearest neighbor sites */
...
public:
	...
	void set_z(int z_spec) {
		z = z_spec;
		NN_site_index.resize(z);
	};

	int get_NN_site_index(int bond_index) const {
		assert(bond_index < z && bond_index >= 0);
		return NN_site_index[bond_index];
	};

	void set_NN_site_index(int bond_index, int i_site) {
		assert(bond_index < z && bond_index >= 0);
		NN_site_index[bond_index] = i_site;
	};
...
public:
	...
	int _NN_of_Site(int site_index, int bond_index) const {
		...
		return LatticeSite[site_index].get_NN_site_index(bond_index);
	};
	...
	void print_info_NN_of_Site(int site_index) const {
		const int z = _z(site_index);
		for (int bond_index = 0; bond_index < z; bond_index++) {
			std::cout << "-- NN at bond " << bond_index << ": ";
			LatticeSite[_NN_of_Site(site_index, bond_index)].print_info();
		}
	};
...
private:
	static const int N_SL_SquareLattice = 1;
	static constexpr int z_SquareLattice[N_SL_SquareLattice] = { 4 };

public:
	SquareLattice(const std::vector<int>& L_spec)
	: Lattice(N_SL_SquareLattice, z_SquareLattice, L_spec) {
		set_network();
	};
	...
	void set_network() {
		for (int i = 0; i < N_sites; i++) {
			...
			for (int bond_index = 0; bond_index < z[sl_index]; bond_index++) {
				...
				LatticeSite[i].set_NN_site_index(bond_index, NN_site_index);
			}
		}
	};
...
int main(int argc, const char * argv[]) {
	...
	if (1) { /* Previous Lecture (2/20/2023) */
		/* square lattice */
		const int N_SL = 1;
		const int dim = 2;
...
//	L[0] = 10;
//	L[1] = 10;
//	SquareLattice SqLatt(L);
//	const int N_sites = SqLatt._N_sites();
//	for (int i = 0; i < N_sites; i++) {
//		std::cout << "NN sites of site " << i << ": ";
//		const int z_i = SqLatt._z(i);
//		for (int bond_index = 0; bond_index < z_i; bond_index++) {
//			const int j = SqLatt._NN_of_Site(i, bond_index);
//			std::cout << j << " ";
//		}
//		std::cout << std::endl;
//	}
